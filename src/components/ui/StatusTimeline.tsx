...
const steps: PickupStatus[] = ['Pending', 'Accepted', 'On the Way', 'Collected', 'Completed'];
        ...
        return (
          <div key={step} className="flex gap-3">
            <div className="flex flex-col items-center">
              ...
              {idx < steps.length - 1 && (
                <div className={`w-0.5 h-10 ${isComplete ? 'bg-eco-400' : 'bg-slate-200'}`} />
              )}
            </div>
            <div className="pt-1 pb-6">
              <p className={`text-sm font-semibold ${isComplete ? 'text-eco-700' : isCurrent ? 'text-slate-800' : 'text-slate-400'}`}>
                {step}
              </p>
              {isCurrent && (
                <p className="text-xs text-eco-600 mt-0.5">In progress...</p>
              )}
              {isComplete && (
                <p className="text-xs text-eco-500 mt-0.5">Completed</p>
              )}
              {isPending && (
